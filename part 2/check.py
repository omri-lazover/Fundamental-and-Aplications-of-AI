...
from ex2_no_nx import InfinitePirateAgent, PirateAgent, ids, OptimalPirateAgent

from new_tests_inputs import new_tests_inputs
...
class PirateStochasticProblem:
    ...
    def run_round(self):
        ...
        if 'turns to go' not in self.state:
            ...
            print("V*:", self.agent.value(self.state))
            self.gamma = self.state['gamma']
            #self.curgamma = self.state['gamma']
            self.state["turns to go"] = 1000
    ...
    def apply_atomic_action(self, atomic_action):
        ...
        if atomic_action[0] == 'sail':
            ...
            self.score += ((2 - self.state['pirate_ships'][ship_name]
                           ['capacity']) * DROP_IN_DESTINATION_REWARD)*self.curgamma
    ...
    def check_collision_with_marines(self):
        ...
        for ship_stats in self.state["pirate_ships"].values():
            if ship_stats["location"] in marine_locations:
                ...
                self.score -= MARINE_COLLISION_PENALTY*self.curgamma
    ...
    def reset_environment(self):
        ...
        self.state["turns to go"] -= 1
        self.score -= RESET_PENALTY*self.curgamma
        return
    ...
    def build_graph(self):
        ...
        """
        n= len(self.initial_state['map'])
        m =len(self.initial_state['map'][0])
        g = nx.grid_graph((m, n))
        nodes_to_remove = []
        for node in g:
...
def main():
    ...
    avg_scores = []
    counter = 1
    for an_input in new_tests_inputs:
        ...
        for i in range(rounds):
            ...
            try:
                ...
                continue
        #avg_scores.append(score/rounds)
        #print("average score: ", score/rounds)
        counter+=1
        #print(avg_scores)
        #break
    print(avg_scores)
